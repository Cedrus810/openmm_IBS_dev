...
换体系后一键完整诊断（PowerShell）：
    python dexp_experiment.py `
        --debug-all `
        --traj <新体系轨迹.dcd> `
        --traj-top <新体系拓扑.cif> `
        --gmx-top <新体系topol.top> `
        --system-xml <新体系system.xml> `
        --ligand-indices <新体系ligand_indices.json> `
        --ligand MOL `
        --output-dir output/dexp_new_system `
        --perturb-anchors 20 `
        --device cuda `
        --platform CUDA

debug-all 顺序：perturb-scan -> production-equivalence-audit ->
alpha-beta-scale-diagnostic -> kernel-projection-benchmark ->
mace-residual-force-benchmark -> 标准六联图 -> mace-env-convergence。
运行状态、耗时、失败步骤和产物路径写入 <output-dir>/debug_all_summary.json。
debug-all 会重新生成 perturb-scan 缓存；程序异常立即停止，科学判据未通过则记录后继续。
...
def load_abfe_symbols():
    try:
        ...
        from dexp_退役 import (  # type: ignore
    ...
    return {
        ...
        "DEXP_VDW_CUTOFF_NM": DEXP_VDW_CUTOFF_NM,
        "DEXP_VDW_SWITCH_WIDTH_NM": DEXP_VDW_SWITCH_WIDTH_NM,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--perturb-baseline-cutoff-nm", type=float, default=0.70, help="计算 DEXP 解析基线 pairwise 和时的截断半径 (nm)，默认与生产 DEXP_VDW_CUTOFF_NM 一致")

    parser.add_argument(
        "--debug-all",
        action="store_true",
        help=(
            "新体系一键完整诊断：依次重跑 perturb-scan、production-equivalence-audit、"
            "alpha-beta-scale-diagnostic、kernel-projection-benchmark、"
            "mace-residual-force-benchmark、标准六联图和 mace-env-convergence；"
            "任一步抛出异常即停止，并持续更新 debug_all_summary.json"
        ),
    )
...
def run_kernel_projection_benchmark(args: argparse.Namespace, output_dir: str) -> Dict:
    ...
    cutoff_nm = float(args.perturb_baseline_cutoff_nm)
    switch_width = 0.20  # 与生产 DEXP_VDW_SWITCH_WIDTH_NM 一致
...
def run_production_equivalence_audit(args: argparse.Namespace, output_dir: str) -> Dict:
    ...
    dexp_cutoff_nm = symbols["DEXP_VDW_CUTOFF_NM"]
    dexp_switch_width_nm = symbols["DEXP_VDW_SWITCH_WIDTH_NM"]
    ...
    def _build_dexp_context(use_switch: bool):
        ...
        force.setCutoffDistance(dexp_cutoff_nm * unit.nanometer)
        force.setUseSwitchingFunction(bool(use_switch))
        if use_switch:
            force.setSwitchingDistance(
                (dexp_cutoff_nm - dexp_switch_width_nm) * unit.nanometer
            )
        for exc_idx in range(nb_force.getNumExceptions()):
            p1, p2, _, _, _ = nb_force.getExceptionParameters(exc_idx)
...
def _debug_all_release_memory() -> None:
    """在一键诊断的阶段边界释放 Python/CUDA 缓存，避免长流程逐步抬高显存峰值。"""
    gc.collect()
...
def _run_debug_all_plot(output_dir: str) -> Dict:
    """用标准六联图脚本绘制当前 output_dir，不复制第二套绘图实现。"""
    plot_script = Path(__file__).with_name("plot_kernel_projection_benchmark.py")
    ensure_file(str(plot_script), "kernel projection 标准绘图脚本")
    env_keys = ("DEXP_BENCHMARK_BASE", "DEXP_BENCHMARK_LABEL")
    old_env = {key: os.environ.get(key) for key in env_keys}
    os.environ["DEXP_BENCHMARK_BASE"] = os.path.abspath(output_dir)
    os.environ["DEXP_BENCHMARK_LABEL"] = Path(output_dir).name
    try:
        plot_globals = runpy.run_path(str(plot_script), run_name="__main__")
    finally:
    ...
    figure_path = str(
        plot_globals.get(
            "OUTPUT_PATH",
            os.path.join(output_dir, "kernel_projection_benchmark_core_figure.png"),
        )
    )
    ensure_file(figure_path, "kernel projection 标准六联图")
    return {"figure_path": figure_path}


def run_debug_all(args: argparse.Namespace, output_dir: str) -> Dict:
    """新体系完整 DEXP/MACE kernel 调试流水线。

    顺序刻意把只读缓存的便宜诊断放在 perturb-scan 之后，把唯一需要额外 MACE
    扫描、成本最高的 env convergence 放到最后。阶段状态每次都原子性地重写到同一个
    summary；科学判据不通过会如实落盘但不会被当成程序异常，只有无法完成计算时才停止。
    """
    summary_path = os.path.join(output_dir, "debug_all_summary.json")
    artifact_paths = {
        "perturb_scan": os.path.join(output_dir, "perturb_scan_summary.json"),
        "production_equivalence_audit": os.path.join(output_dir, "production_equivalence_audit_summary.json"),
        "alpha_beta_scale_diagnostic": os.path.join(output_dir, "alpha_beta_scale_diagnostic_summary.json"),
        "kernel_projection_benchmark": os.path.join(output_dir, "kernel_projection_benchmark_summary.json"),
        "mace_residual_force_benchmark": os.path.join(output_dir, "mace_residual_force_benchmark_summary.json"),
        "kernel_projection_figure": os.path.join(output_dir, "kernel_projection_benchmark_core_figure.png"),
        "mace_env_convergence": os.path.join(output_dir, "env_convergence_summary.json"),
    }
    steps = [
        ("perturb_scan", run_perturbation_scan),
        ("production_equivalence_audit", run_production_equivalence_audit),
        ("alpha_beta_scale_diagnostic", run_alpha_beta_scale_diagnostic),
        ("kernel_projection_benchmark", run_kernel_projection_benchmark),
        ("mace_residual_force_benchmark", run_mace_residual_force_benchmark),
        ("kernel_projection_figure", lambda _args, out: _run_debug_all_plot(out)),
        ("mace_env_convergence", run_mace_env_convergence),
    ]
    summary: Dict = {
        "status": "running",
        "protocol": "new-system DEXP kernel debug-all v1",
        "output_dir": os.path.abspath(output_dir),
        "perturbation_protocol": {
            "anchors": int(args.perturb_anchors),
            "translation_nm": args.perturb_trans_nm,
            "rotation_deg": args.perturb_rot_deg,
            "random_translation_directions": int(args.perturb_n_random_dirs),
        },
        "environment_convergence_protocol": {
            "anchors": int(args.env_convergence_anchors),
            "radii_nm": args.env_convergence_radii,
        },
        "steps": [],
    ...
    print("[debug-all] 开始新体系完整诊断；将重建 perturb-scan 缓存，环境收敛在最后运行。")
    for index, (name, runner) in enumerate(steps, start=1):
        step = {"index": index, "name": name, "status": "running"}
        summary["steps"].append(step)
        _write_summary()
        print(f"\n[debug-all {index}/{len(steps)}] {name}")
        start = time.perf_counter()
        try:
            runner(args, output_dir)
            step["status"] = "completed"
            step["elapsed_seconds"] = float(time.perf_counter() - start)
            step["artifact"] = artifact_paths[name]
        except Exception as exc:
            step["status"] = "failed"
            step["elapsed_seconds"] = float(time.perf_counter() - start)
            ...
            summary["elapsed_seconds"] = float(time.perf_counter() - total_start)
            _write_summary()
            print(f"[debug-all] 失败于 {name}: {type(exc).__name__}: {exc}")
            raise
        finally:
            _debug_all_release_memory()
        _write_summary()

    summary["status"] = "completed"
    summary["elapsed_seconds"] = float(time.perf_counter() - total_start)
    _write_summary()
    print(f"\n[debug-all] 全部完成。总控汇总: {summary_path}")
    return summary
...
def main() -> int:
    ...
    # 新体系一键完整诊断。必须位于所有单模式分支之前；传入的其它模式开关在此模式下不执行。
    if args.debug_all:
        run_debug_all(args, output_dir)
        return 0
