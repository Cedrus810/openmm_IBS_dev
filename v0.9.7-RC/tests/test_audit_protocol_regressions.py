...
class SourceContractTests(unittest.TestCase):
    ...
    def test_learning_to_freeze_freezes_only_when_step_settled(self):
        # [IBS_BIAS_PROTOCOL_VERSION=29] freeze 时机（纯效率、不阻放行）：只看 f_k 的
        # 应用 pairwise 步长是否已降到 ≤ IBS_TMBAR_FREEZE_MAX_APPLIED_PAIRWISE_STEP_KT
        # kT（f_k 几乎不再动），连续 N 批才冻结去跑纯 Δf−ΔF 门。故意不看占据平坦/
        # coverage/abs_ess——那会把宽松门变严格。step 门不阻挡任何本该通过的窗口
        # （gap<10 ⟹ 残差小 ⟹ 步长小）。
        self.assertIn("IBS_LEARNING_READY_CONSECUTIVE = 2", self.engine)
        self.assertIn("learning_ready = bool(", self.engine)
        self.assertIn(
        ...
        # 占据平坦/coverage 硬门已从 freeze 时机删除（曾是隐藏的严格门）。
    ...
    def test_vanishing_uses_thermodynamic_few_state_subdomains_without_overlap_two(self):
        ...
        for _bad in (
            ...
            self.assertNotIn(_bad, self.pipeline)
