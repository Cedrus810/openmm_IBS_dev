...
def _fit_dexp_with_ml_model(
    ...
    if holdout_idx.size > 0 and fitted_params.get("fitting_success"):
        ...
        holdout_delta_e = [accepted_delta_e_final[i] for i in holdout_idx]
        ...
        holdout_rows = [
            ...
            for frame_id, actual, predicted in zip(holdout_frame_ids, holdout_delta_e, predicted_holdout)
        ]
        ...
        try:
            ...
            ax.scatter(actual_arr, predicted_arr, s=14, alpha=0.6)
            ...
            ax.set_xlabel("Actual delta-E (Orb reference, kJ/mol)")
            ax.set_ylabel("DEXP predicted delta-E (kJ/mol)")
            ax.set_title(
                f"Holdout parity (n={len(actual_arr)}, R2={holdout_metrics['r2_raw']:.3f}, "
                f"RMSE={holdout_metrics['rmse_raw_kjmol']:.1f} kJ/mol)"
            )
            ax.legend()
            ax.grid(alpha=0.3)
        ...
        print(
            f"    留出集验证: n={holdout_metrics['n_holdout_frames']} | "
