...
**NVT**：历史参考运行 `output_lrc_fix/`（Atenolol-rank11，不在本分支）的
`system_native.xml` 里本来就没有 barostat（已核），
脚本的 `_strip_barostats` 只是保险。⟨ΔU⟩ 是同一帧上两个 Hamiltonian 之差，
与系综是 NVT 还是 NPT 无关。

用法::

    python tools/diagnostics/diagnose_boresch_ensemble_effect.py --run-dir output \\
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-dir", default=DEFAULT_RUN_DIR, help="复合物缓存目录（只读）")
