...
更新日期：2026-07-22（新增 VAL-GPU-008）；2026-07-21（新增 VAL-TEST-003）；2026-07-19（旧自动修复验证结项，新增 VAL-GPU-007/VAL-NM-001）
...
| `VAL-GPU-007` | [`#28`](https://github.com/Cedrus810/openmm_IBS_dev/issues/28) | 在线 IBS 学习以 TMBAR 取代朴素 TA（`IBS_BIAS_PROTOCOL_VERSION=19`） | 已修复 | `py_compile` 通过；TMBAR 解析回归已补；尚未在目标 pytest/GPU 环境实跑 | OpenMM + CUDA + PyMBAR | `待运行` / Project `In review` | 2026-07-19 |
...
| `VAL-TEST-004` | [#33](https://github.com/Cedrus810/openmm_IBS_dev/issues/33) | Stage 2 v18：17 个 Fisher 常规节点 + 4 个端点插入节点；闭区间窗口 `[0,5] [5,9] [9,13] [13,17] [17,20]` | 已修复 | AST、源码契约和旧 pilot 离线数学回归通过：21 唯一 λ、25 槽位、相邻仅共享 5/9/13/17 | OpenMM + CUDA + PyMBAR + pytest | `待运行` / Project `In review` | 2026-07-21 |
...
| `VAL-GPU-008` | — | 2026-07-22 IBS production 协议：TI/TMBAR 符号、一次 fixed-f 预热验证、production `f_k` 硬锁、预热/生产数据隔离、失败窗口精确定位、定向 production 续采及 immutable rescue ensembles | 已修复 | AST 通过；源码契约 32/32；失败定位与 rescue ranges 纯函数测试通过 | Linux + OpenMM + CUDA + PyMBAR；复用真实 Stage 2 失败输出 | `待运行` | 2026-07-22 |
