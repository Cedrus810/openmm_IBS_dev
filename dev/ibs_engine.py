...
class GlobalMBARAnalyzer:
    ...
    def solve_stage_integrated(
        ...
        # 一阶矩伴随门：ESS 尺度不变，抓不到"均匀被饿死"的态（实测一个 f_k 未补偿、
        # 高出 80 kT 的态 ESS/N 仍报 0.34）。同样要求全部窗口都算得出来才算证据齐全。
        occ_vals = [
        ...
        converged = bool(
            ...
            and min_occupancy_normalized is not None
        ...
        return {
            ...
            "ess_gate_protocol_version": int(ESS_GATE_PROTOCOL_VERSION),
            "min_occupancy_normalized": min_occupancy_normalized,
            "min_occupancy_normalized_threshold": min_occupancy_normalized_threshold,
            ...
            "uncertainty_note": (
                ...
                "窗口段方差；同一窗口内的端点协方差因此不会重复丢失。converged 同时要求 "
                "ESS ratio、绝对 ESS、去相关样本数、端点差不确定度四项达标。不同窗口"
                "来自独立采样，未假设跨窗口协方差。"
            ),
            "f_k": f_curve.tolist(),
    ...
    def __init__(
        ...
        if max_resident_contexts is None:
            max_resident_contexts = (
                1
                if requested_platform.upper() in {"CUDA", "OPENCL"}
        ...
        if (
            ...
            print(
                "  ⚠️ REMD replica 数超过 GPU 常驻 Context 上限 "
                f"({self.n_replicas}>{self.max_resident_contexts})；"
