            ...
            if resume:
                ...
                if os.path.exists(energies_path) and os.path.exists(convergence_path):
                    try:
                        ...
                        cached_lse_tolerance = cached_conv.get(
                        ...
                        # 🔑 [non_mutating_v1] 采样修复策略必须匹配：旧的变异策略缓存
                        # （其 f_k 可能被 fixed-H 累计 ΔF 就地覆盖过，属于不同参考系）
                        # 绝不能被非变异策略的 run 复用。旧缓存没有这个字段（None），
                        # 与 "non_mutating_v1" 不相等，因此自动判为无效并重新采样。
            ...
            if (
                ...
                if 0 < prior_cumulative_production_steps < effective_n_steps_per_window:
                    if _try_load_main_window_checkpoint(sim, production_ckpt_path):
                        try:
                            _prior_e = np.load(production_energies_path)  # (K, N)
                            _prior_b = (
                                np.load(production_bias_path)
                                if os.path.exists(production_bias_path)
                                else np.zeros(_prior_e.shape[1])
                            )
                            _prior_base = (
                                np.load(production_base_path)
                                if os.path.exists(production_base_path)
                                else np.zeros(_prior_e.shape[1])
                            )
            ...
            for up in range(n_updates):
                ...
                if (up + 1) % 100 == 0:
                    ...
                    if sampler.bias_history:
                        _atomic_save_npy(production_bias_path, np.array(sampler.bias_history))
                    if sampler.base_energy_history:
                        _atomic_save_npy(production_base_path, np.array(sampler.base_energy_history))
                    _periodic_conv = {}
...
class TraditionalMBARAnalyzer:
    ...
    def compute_u_kn(
        ...
        if needs_traditional_lrc:
            ...
            # BeutlerSoftcoreBuilder.build() defaults: the LJ term is scaled
            # by lambda_vdw**1 and D(r)=0.5*(1-lambda_vdw)**1+r**6 (apart
            # from its negligible 1e-4 safety floor near r=0).  Use the same
            # v2 integrator as the dual-lambda/IBS path so producer and worker
            # share both the key and the physical definition.
            ...
            print(
                "  ✅ 传统 Beutler LRC: v2 switching+softcore-aware "
                "r^-6/r^-12 离线尾项已启用 "
