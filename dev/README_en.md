...
warm-up validation, the candidate `f_k` is frozen; the residual is an efficiency
diagnostic rather than an indefinitely repeated production admission gate. Production
starts from its own step zero without warm-up/validation frames, and a runtime lock
makes `f_k` read-only throughout production. On a Stage 2 quality-gate failure, the
default cumulative production budgets are 250k -> 500k -> 1M for failing windows only;
if that is still insufficient, a separate rescue ensemble is created. See
`docs/status/IBS_PRODUCTION_PROTOCOL_2026-07-22.md` for the complete protocol and
pending validation scope. There are currently
no confirmed P0/P1 blockers on the default production path. See `docs/TODO.md`
for unfinished code or engineering actions and `docs/status/VALIDATION_MATRIX.md` for code-complete fixes
...
docs/TODO.md            Single current action list
docs/status/            Audit history, closed status, and validation matrix
docs/handoffs/          Topic-specific debugging and experiment handoffs
todo2.txt               Compatibility pointer to docs/TODO.md
```
