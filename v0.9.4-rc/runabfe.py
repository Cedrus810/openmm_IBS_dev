...
def run_post_analysis(args):
    ...
    def _analyze_dual_leg(base_dir: str) -> Dict:
        ...
        for stage in stages:
            ...
            if os.path.exists(stage_checkpoint):
                ...
                total_err_sq += float(cached_stage.get("total_error", 0.0)) ** 2
                continue
            ...
            if not e_files and not os.path.exists(stage_checkpoint):
    ...
    # ✅ --analyze-only 此前从未应用 --apbs-correction-kj-mol，即便该 CLI 参数在
    # 这里同样存在（见上方 parser 定义）；与主流程 (runabfe.py 正式 total/APBS 组装)
    # 口径保持一致，补上这项外部长程修正。
