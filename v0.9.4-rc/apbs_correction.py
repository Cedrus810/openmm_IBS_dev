...
def _write_apbs_input(
    ...
    body.append(_write_elec_block(
        "protein_RIP_het", 1, dime, box_a, center_a, temperature_k, epsilon_s,
        use_membrane_maps=(mode == "membrane_continuum"),
    ))
    body.append(_write_elec_block(
        "ligand_RIP_het", 2, dime, box_a, center_a, temperature_k, epsilon_s,
        use_membrane_maps=(mode == "membrane_continuum"),
    ))
    body.append(_write_elec_block(
        "ligand_RIP_hom", 2, dime, box_a, center_a, temperature_k, 1.0,
...
def _potential_to_integrated_potential_kj_nm3_per_mol_e(
    ...
    """Follow RocklinC: mean APBS potential (kT/e) times N*voxel volume."""
    geometry, values = _read_dx_values(path)
    average_kT_per_e = statistics.fmean(values)
    grid_volume_nm3 = geometry.voxel_volume_a3 * len(values) / 1000.0
