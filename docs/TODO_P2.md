...
- [ ] **S2-A [P2] solver 侧证据在整个采样阶段恒空 ⟹ 三条判据不带电（2026-09-17 真机 ×3）**
  ...
  **2026-09-17 已做的那一半（只消除静默，行为一字未改）**：
  ...
  完整记录 → [STAGE2_CONTROLLER_WAVE_2026-09-17.md](archive/STAGE2_CONTROLLER_WAVE_2026-09-17.md) §3.A

> ### ✅ 已关闭的 4 条整段移进 [archive/TODO_closed.md](archive/TODO_closed.md)（2026-09-18 搬）
> 
> | 条目 | 一句话 |
> |---|---|
> | `S2-B` | 缺窗覆盖被纯补帧挡住 |
> | `S2-E` | 插 λ 对末窗是空动作，且边际停滞分支不考虑拆窗 |
> | `S2-I` | 末窗被推到 `K == hi` 就卡死：既不拆、也再插不进 |
> | `S2-F` | 布局动作没有任何事后有效性检查 |
...
- [ ] **IDENT-01 [P2] `allow_untrusted_stage_results` 在顶层 `final_results` 指纹里，stage 指纹里已剔除。**
  ...
  **两边口径不一致是事实，但哪边对没定。** 两个方向都有像样的理由，别只看一边就改：
  ...
  - **不该剔**：顶层指纹代表的是**整个 run 的身份**，而这个开关确实改变了
    ...
    （见 [archive/TODO_closed.md](archive/TODO_closed.md)）。
...
[RELEASE_READINESS_2026-08-31.md](archive/RELEASE_READINESS_2026-08-31.md)。
...
> 下面三条 2026-09-13 从 [RELEASE_READINESS_2026-08-31.md](archive/RELEASE_READINESS_2026-08-31.md)
