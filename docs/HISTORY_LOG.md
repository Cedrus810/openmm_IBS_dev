...
[当前行动清单](TODO.md) 和 [发布准备度评估](archive/RELEASE_READINESS_2026-08-31.md)。
...
## 六、已过期的「主线体系」声明（2026-09-18 从 STATUS.md 移出）
...
**2026-09-18 判定：完全过期，移出。** 当前全部在推的工作都在
**13 体系 × 3 rep 的 benchmark**（cmet / cyclod / jnk1 / p38 / brd4 / thrombin）——
`TODO.md` 的 `BM-B`、`AUDIT-S2-03`，以及 09-16～09-18 三批真机，没有一条是 4W53。
4W53 只剩 `STATUS.md` 结果登记表里的一行（`output_v3_seed20260908`，单 seed，
本仓库内无第二个独立重复，**不可作为最终结论引用**）。

原节里的**符号约定**（`Delta G_bind = Delta G_solvent - Delta G_complex + Delta G_APBS`）
仍然有效，留在 `STATUS.md`，已独立成节。

---

## 七、2026-09-14 控制器修复快照（2026-09-18 从 STATUS.md 移出）

> 原节标题：「下面这节是 2026-09-14 当天的快照（保留原文，别改）」。
> 它是**当天**的状态，不是当前状态，所以从 `STATUS.md` 移到这里。原文一字未改。
...
| 六路并行全面审计 | **63 条**缺陷（三条贯穿性根因：路由在知道窗口缺什么之前就锁定 / 「未知」四处四套语义 / 子窗是二等公民） | **65 条（63 + 事后 2）全部落地**（2026-09-16 逐条回源码对账；状态列曾虚挂 20 条 `OPEN`）。逐条见 [archive/CONTROLLER_BUDGET_AUDIT_2026-09-14.md](archive/CONTROLLER_BUDGET_AUDIT_2026-09-14.md)。**真机验收一条都没跑过** → `TODO.md` `AUDIT-S2-03` |
...
> 🔴 **2026-09-16 更正（本条原先停在 09-14 的「18 条仍 `OPEN`」，那是过期的）**：
> 按审计文档自己定的判据（源码里的 `[审计 #N]` 标记）逐条回源码对账，**65 条全部落地** ——
> 包括原先被点名「会炸整跑」的 `#11`/`#12`/`#13` 与「会静默产出错误结果」的 `#2`/`#4`。
> 那 20 条虚挂的 `OPEN` 只是没人回来落标记，**代价不是漏修，是重复排查**
> （`TODO.md` 的 `BUD-*` 有一部分就是在已修的地方重新查出来的）。
>
> ⟹ **规矩：「某条缺陷修没修」的权威只有源码**，审计文档的状态列和 `- [ ]` 都是会过期的缓存。
>
> ⚠️ 但「缺陷不在了」**不等于「修得对」** —— 65 条仍是**真机零验证**（`TODO.md` `AUDIT-S2-03`），
> 三条贯穿性根因也没有过期（`AUDIT-S2-02` 就是根因 1 还活着的证据）。
> 逐条见 [archive/CONTROLLER_BUDGET_AUDIT_2026-09-14.md](archive/CONTROLLER_BUDGET_AUDIT_2026-09-14.md)。
