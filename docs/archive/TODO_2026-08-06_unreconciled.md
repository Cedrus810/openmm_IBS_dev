# 已归档：2026-08-06 行动清单（未对账）

> **状态：只读历史存档，不是当前待办。**
>
> 这是 `docs/TODO.md` 截至 **2026-08-06** 的主表，2026-08-31 发布整理时整段移到这里，
> 目的是让活的 `docs/TODO.md` 回到可读长度（原本 1631 行）。
>
> **内容一字未改，包括那些 `- [ ]` 勾选框。它们只表示"2026-08-06 当时未完成"，
> 不表示现在仍未完成——相当一部分很可能已经做完了。**
> 本仓库里没有能证明哪些做完了的判据，判据在 `Atenolol-rank11`：
>
> | 材料 | 内容 |
> |---|---|
> | `archive/patches/BUGFIX_HANDOFF_2026-08-29_resolved_issues.md` | 41 项已解决的逐条记录 |
> | `0831issue.md` | 第九轮代码审查新发现（2026-08-31，尚未完成） |
> | `github issue.md` | GitHub issue 状态 |
>
> ⚠️ **不要按编号对账。** 两套编号同名不同义：`P1-19` 在 08-29 那份里是
> "v4 charging 接缝内静电失配"（已修），在**本文件**里是
> "per-window σ 系统性低估 2–4 倍"（当时标为最高优先物理项）。
> 本文件用 `ATT-xx` / `MEM-xx` / `P0-9~13` 一套，那份用 `P1-xx` / `P2-xx` / `PHY-xx` 一套。
>
> 当前待办见 [../TODO.md](../TODO.md)。

---

原始说明（2026-08-06）：完整 2026-07-27 审计长记录已移入 [archive/TODO-2026-07-27-full.md](../HISTORY_LOG.md)。历史原文见 [archive/todolist-2026-07-20.md](../HISTORY_LOG.md)，审计证据见 [status/AUDIT_STATUS.md](../HISTORY_LOG.md)，运行验证见 [status/VALIDATION_MATRIX.md](../HISTORY_LOG.md)。本轮移出的完成/关闭项见 [../archive/todo-2026-07-29.md](../HISTORY_LOG.md)。

- **B4（溶剂腿 charge-transfer builder）已落地（2026-08-05）。** 详见 `docs/status/memtodolist.md`
...
- **BORESCH-ORDER-01：窗口预热在 EM 之后才打开 Boresch 限制力，排序反了，已修
  ...
  生产窗口本来就是在 scale=1.0 下采样完成的，不受影响。详见 `docs/status/memtodolist.md`
...
- **C1（带电小水盒验证）改用单原子离子当"配体"，脚本已写完（2026-08-06）。**
  `docs/status/memtodolist.md` 原稿 C1 要求质子化 Atenolol(+1)/acetate(-1) 这类现造小分子，
  ...
  详见 `docs/status/memtodolist.md` §17.0/C1 与脚本头部 docstring。
...
- **Boresch 二面角符号反号根因已修（BOR-01，2026-07-29）。** `abfe_core.py` 四份手写
  ...
  [handoffs/BORESCH_DIHEDRAL_SIGN_HANDOFF.md](../HISTORY_LOG.md)。
...
- [ ] **P1-19b（已知统计限制，生产验收前处理；**不**阻塞工程闭环与 B3/B4/B5 开发）：
  ...
  **不要**用"多跑几次取平均"当作 σ 口径的修复。
  与 P1-19 的关系：`docs/status/memtodolist.md` §17 规定 P1-19 阻止**进生产**，
  并没有规定它阻止 B3/B4/B5 的方法开发 —— 别让它把工程闭环拽住。
  详见 `docs/status/memtodolist.md` §0.5.12。
...
- [~] **ATT-22：分层 CPU CI 与静态检查已在本地实现，待推送后首轮 Actions 验收。** GitHub [#62](https://github.com/Cedrus810/openmm_IBS_dev/issues/62)。新增 `.github/workflows/cpu-ci.yml`、`environment-ci.yml`、`pyproject.toml` 与质量合同；普通 CI 排除 GPU 作业。`abfe_core.py`、`abfe_pipeline.py`、`abfe_preoptimizer.py` 当前裸 `except:` 为 0。Ruff/语法/Black/isort/渐进 mypy 本地门已通过；关闭前需提交、推送并确认 Actions 首轮全绿。
...
- [ ] **MEM-00：膜受体–配体 ABFE 专项清单已成文（2026-07-29）。**
  设计与验收清单在仓库根的 [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线)，本轮补入了对现有代码的
...
- [x] **膜体系第一次端到端跑通（2026-08-04）；§15 成本已按实测关闭。**
  ...
  实测耗时/占用见 [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线) §15（已由估算改成实测表）：
...
- [x] **P0-13（2026-08-04 已修，P0-12 的真正根因）。**
  ...
  [handoffs/MEMBRANE_SOLVENT_LEG_P013_HANDOFF.md](../HISTORY_LOG.md)。
  正文：**抽配体参数时对同类型力做了
...
- [x] **B3 + MEM-00d：PME charge-transfer charging Hamiltonian 已落地（2026-08-04）。**
  主线 §17.0 的第 ② 步。`docs/status/memtodolist.md` Phase B3 与 MEM-00d 绑定完成（restraint 形式
  ...
  * co-ion 的 §13.1 几何判据在**构造时**只强制了"离配体够远"这一条；
  ...
  [handoffs/CHARGE_TRANSFER_B3_HANDOFF.md](../HISTORY_LOG.md)。
...
- [x] **MEM-00i / Phase B1 已实现（2026-07-30）：`system_type=membrane` + 膜恒压器。**
  ...
  详见 [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线) 的 MEM-00i / §3.1 / §3.2 / §7.4 / B1。
...
- [ ] **MEM-RUN：memtest 膜体系首跑进展（2026-07-30）。**
  ...
  [`memtest/README_MEMTEST.md`](../HISTORY_LOG.md)。

  **已跑通到预平衡**：10 ns NPT 完成（480 ns/day），末段 T 303.6–305.8 K、
  PE −508k~−510k kJ/mol 无漂移、体积 438.97–440.37 nm³、密度 1.034 g/mL。

  沿途修掉的 5 个真缺陷（全部记在
  [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线) §0.5.4–§0.5.8，每条都有 file:line 与实测证据）：
...
- [x] **MEM-15：刚性水被 PBC 修复撕开 → Stage 0 的 NaN（2026-08-03 已修）。**
  ...
  **证据**：`tests/test_membrane_barostat_protocol.py::
  ...
  详见 [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线) §0.5.10。
...
- [ ] **MEM-BONDS：mmCIF 拓扑缓存丢掉非标准残基的键（可溶路径同类隐患待评估）。**
  ...
  **膜体系已修**（`require_bonded_topology=True` 优先从 `.top` 重建 + 键数校验 +
  最小化后受力合理性门），详见 [当前 TODO 的《膜受体–配体路线》一节](../TODO.md#膜受体配体路线) §0.5.7。
