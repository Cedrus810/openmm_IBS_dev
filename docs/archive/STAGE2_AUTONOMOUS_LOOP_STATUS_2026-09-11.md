...
> ## 🗂 2026-09-18 归档 —— **不是待办**
>
> 09-11 那批 bug 的修复流水账，**那批活全部做完**。接手 stage-2 现在读
> [STAGE2_CONTROLLER_DESIGN_2026-09-12.md](STAGE2_CONTROLLER_DESIGN_2026-09-12.md)
> （控制器 09-12 首次独立走完六窗），当前开着的缺口在
> [../TODO.md](../TODO.md) / [../TODO_P2.md](../TODO_P2.md) 的 `S2-*`。
>
> | 本文的哪一段 | 状态 |
> |---|---|
> | §9「把预算改名叫要求」 | **仍然有效** —— `ibs_engine.py` 的 `minimum_complete_validation_frames` 注释直接引它（"差 7.5 倍不可达"那个误判的唯一出处） |
> | 开头引的核心指标 | 原件是 [AUTONOMOUS_STAGE2_LOOP_SPEC_2026-09-11.md](AUTONOMOUS_STAGE2_LOOP_SPEC_2026-09-11.md)（同批归档），指标本身已抄进控制器设计文档页首 |
> | 其余「剩余工作」清单 | **已过期**，别当待办读 |
...
> **2026-09-13 已修**（`S2-B`）：预算那一支已从公式里去掉。
> ⚠️ 但上面这段诊断有一处要更正：**这个量在本仓从来没有当过门**（自第一个
> commit 起就只进报告），可达性预检的 T 读的是 `decorrelated_frames_required`。
> 所以「规则 2 的第 2 档过不去」这个后果**没有真的发生过** —— 真实危害是这个数
> 会骗读它的人（T 一度就被错取成它），以及两次 run 的报告没法横向比。
