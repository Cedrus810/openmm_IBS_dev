...
def pme_offset_charge_square_sum(
    ...
    for offset_idx in range(nb_force.getNumParticleParameterOffsets()):
        ...
        if str(param) != str(lambda_name):
            continue
        q_val = charge_scale.value_in_unit(unit.elementary_charge)
        qsq += q_val * q_val
        rows.append({
...
class IBSWindowManagerDualLambda:
    ...
    def _gradual_warmup_debug(self, sim, ibs_wrap, sampler, warmup_steps, win_sys, window_idx, debug_mode=True):
        ...
        total_steps = max(0, int(warmup_steps))
        chunk = min(500, total_steps) if total_steps > 0 else 0

        if total_steps <= 0:
            sim.context.setParameter(f"{self.prefix}_bias_scale", 1.0)
            print("  ⏭️  跳过纯偏置预热 (warmup_steps <= 0)")
            return
...
class GlobalMBARAnalyzer:
    ...
    def solve_stage_integrated(self, window_data: List[Dict]) -> Dict:
        ...
        for local in local_results[1:]:
            ...
            offset_var = float(np.sum(offset_vars) / (len(offset_vars) ** 2))
        ...
        return {
            ...
            "method": "Local-TMBAR-Stitched (offset-error-propagated)",
            "uncertainty_note": (
                "局部 MBAR 拼接误差已传播窗口 offset 方差；仍未包含完整全局 MBAR "
                "协方差矩阵、自相关时间和有效样本数修正。"
            ),
            "f_k": f_curve.tolist(),
...
class TraditionalMBARAnalyzer:
    ...
    def compute_u_kn(
        ...
        if is_pme_coulomb_leg:
            ...
            if offset_qsq > 0.0:
                ligand_charge_square_sum = offset_qsq
                ...
                print(
                    "  ✅ PME 自能修正启用: "
                    f"Σq_offset²={offset_qsq:.6f} e²，包含 lambda_coul 控制的全部粒子 offset。"
                )
