...
> **本文件角色：完整历史归档。** 这是 2026-08-07 精简
> [`IMPLEMENTATION_PLAN_outer_lambda_neural_basis.md`](IMPLEMENTATION_PLAN_outer_lambda_neural_basis.md)
> 之前的完整快照，包含全部已完成/已终止工作包的时间线、通过门和数值证据。当前仍在
> 生效的目标、约束、设计规范和尚未完成的工作以主文件为准；本文件只用于查证据，不再
> 更新维护。
>
> **原文档角色说明（历史）：工程执行计划。** 科学原则见
...
**D3-0 provenance gate 已关闭（DEC-041）**：`win_sys_xml_sha256_matches_manifest=false`
经 `scripts/verify_exp012_win_sys_provenance_d3_0.py`（report SHA-256
`2dc557092ce327c8af3eb2d137c489817a0267377604b077e4469b4d54ba32a8`）判定为
`CLOSED_STEP3_OPERATIONAL_SEMANTIC_PASS`：同进程两次重建确定性一致；换用真实
`resolve_dispersion_protocol`/`resolve_membrane_protocol`（结果与此前硬编码值相同，
`legacy_uniform_density_lrc`/`soluble`）后哈希仍不匹配，但 10 项独立字段核对
（masses/constraints/virtual sites 对 `system_native.xml`，lambdas/温度对
`manifest.json`，potential_type/Boresch/softcore 对 `stage2_vanishing.json`，IBS
prefix 对 `ibs_state`，box vectors 取自已加载 checkpoint）与 Force 数量/类型/group/
表达式的 canonical fingerprint 全部一致，未发现语义差异。按预先冻结的判据，结论只
记为"记录里没有可检测出的语义差异；历史 byte-level 不一致视为非阻塞"，不猜测具体
是序列化顺序还是浮点文本格式差异。`no_student_window0_baseline_v2.json`
（report_sha256 `6969d3d0b5316fca6f605beb23416a125433d54f41a825f6df484b4f97e651aa`，
median/P95 `1.3959/1.3968` ms/step）现在可采信为无 student 生产基线，不再是
provisional。System 身份门解除，D3 中依赖真实生产 System 的部分（OpenMM 注入、
student/no-student 正式耗时比较）可以开始；协议只跑一次，不再开第二轮 provenance
调查去猜具体机制。

#### D3 sub-items 1–4 现状（DEC-042，2026-08-06，未关闭）

D3 未全部通过，本轮工作在此暂停（不进入 D4）：

| sub-item | 状态 | 证据 |
|---|---|---|
| 1. deployment 一致性（eager vs scripted，CPU64/CPU32/CUDA32） | **未关闭** | 三类容差拆分后（correctness=1e-8/precision_envelope=5e-4/device_consistency=1e-4），CPU64-vs-CPU64 残差从 `1.415e-05` 降到 `1.691e-07`（修复测试脚本自身的坐标 precision-lineage bug）；另发现 `student_deploy.py::_radial_basis` 用 `.pow(2)` 而 `student.py` 用 `.square()`，已改为 `.square()`，**修复尚未经真实数据重新验证** |
| 2. TorchForce/OpenMM Reference 注入 | **通过** | `all_passed=true`，`torchforce_consistency` 误差 `0.000e+00`，report_sha256 `d71fe52e5b65696e29fb9777d91da612dd87fa3a75f212d4f66e5b0908c8bf88` |
| 3. 端点归零 | **通过** | `endpoint_zeroing` 误差 `0.000e+00`（同一报告） |
| 4. 生产 Context 耗时 | **未关闭** | all-pairs 近邻发现（O(n_ligand×n_system)）经 `scripts/profile_exp012_student_torchforce_overhead_d3.py` 确认单独耗时 7.68ms/call，是网络数学本身（0.28ms/call）的约 27 倍，主导替换前 258% 开销；已替换为周期性 linked-cell list（`local_residual/student_deploy.py::_cell_list_candidates`，box 对角且每轴≥3 bins 时启用，否则退化到原 `_brute_force_candidates`，正确性不依赖盒子形状假设）；替换后开销从 258% 降到 **95%**（report_sha256 `02ecc15244eee8d0d0c68c3de8dcee15fa78c9710ac7e08cb93def9181d6abb1`），仍高于 §3 表冻结的 ≤50% 淘汰门 |

sub-item 4 再次跑 profiling 定位新瓶颈时发现：profiling 脚本自身的 graph-construction
计时函数是替换前旧逻辑的手写镜像副本，未随 `student_deploy.py` 一起更新，测的是已经
废弃的旧代码——已修复为直接调用 `deployable_device` 对象的真实方法，**修复后的
profiling 脚本尚未重新执行**。新增 `tests/test_exp012_student_deploy_cell_list.py`
（纯 CPU、合成几何，验证 cell list 与 brute force 找到完全相同的候选边/距离/前向
反向能量力、TorchScript 可导出性），**尚未运行**。

**下一步（未执行，按顺序）**：① 跑新增单元测试；② 用 `.square()` 修复后重跑 sub-item
1；③ 用修好的 profiling 脚本重新定位当前 95% 开销的新瓶颈；④ 从头重跑完整 D3（三份
脚本）确认全部通过且开销 ≤50%（目标 ≤15%）后才能登记 D3 关闭、开始 D4。详见
EXPERIMENT_LOG DEC-042。
