...
def insert_lambda_from_overlap_failure(
    ...
    if not failed_pairs:
        raise RuntimeError("最小窗口失败诊断中没有 fixed-lambda overlap 失败边，拒绝插点")
...
class DualLambdaPreOptimizer:
    ...
    def optimize_stage2_vanishing(
        ...
        max_window_thermodynamic_length=6.0,
        overlap_thermodynamic_length=1.5,
        max_states_per_window=6,
    ):
        ...
        pilot_lambdas = np.linspace(1.0, 0.0, n_states)
        metric_g = []
        pilot_points = []
        ...
        optimized_lambdas, cumulative_length, optimized_edge_lengths = (
        ...
        window_ranges = partition_windows_by_thermodynamic_length(
            optimized_edge_lengths,
            max_window_length=float(max_window_thermodynamic_length),
            overlap_length=float(overlap_thermodynamic_length),
            max_states_per_window=(
                int(max_states_per_window) if max_states_per_window is not None else None
            ),
        )
        optimized_lambdas = np.asarray(optimized_lambdas, dtype=float).ravel()
        optimized_lambdas = np.clip(optimized_lambdas, 0.0, 1.0)
        ...
        diagnostics = {
            ...
            "max_window_thermodynamic_length": float(max_window_thermodynamic_length),
            "overlap_thermodynamic_length": float(overlap_thermodynamic_length),
            "max_states_per_window": (
                int(max_states_per_window) if max_states_per_window is not None else None
            ),
        ...
        print(
            ...
            f"{len(optimized_lambdas)} 态, {len(window_ranges)} 窗口"
        )
