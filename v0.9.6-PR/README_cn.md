...
[中文（当前页）](README_cn.md) | [English](README_en.md)
...
- 默认 ACE/`dual_lambda` 的 VDW/vanishing 腿已启用解析 LJ 长程尾项修正（`traditional_lj_lrc_protocol_version=2`：对每个 λ_vdw 数值积分真实的 switching-aware（补上 1.0–1.2nm 被 switching 削弱的能量）+ softcore-aware（真实 softcore 分母，非纯 `r^6`）尾项，同时含吸引 `r^-6` 和排斥 `r^-12` 两项；不使用会把同一表达式中 Coulomb 尾项也错误积分、并可能令 CUDA 崩溃的 OpenMM `CustomNonbondedForce` 内建 LRC）。`single_lambda`/REMD 的 Beutler 路径现在使用同一套公式作离线修正；`traditional_lj_lrc_protocol_version<2` 的旧输出对应尚未包含 switching 区间修正的公式，不应与新协议结果混用。
...
- Stage 2 采用 `Local-TMBAR-Stitched`，误差已传播窗口 offset 方差（复合物腿 `offset_error_contribution ≈ 0.52 kJ/mol`，溶剂腿 `≈ 0.82 kJ/mol`），但尚未包含完整全局 MBAR 协方差、自相关时间和有效样本数修正；复合物腿 `min_overlap_proxy ≈ 0.027`，溶剂腿 `≈ 0.043`，偏低，建议后续加密窗口/增加采样再确认。
- 尚未做独立重复运行：`diagnostics.independent_repeats.performed = false`。

更详细的方法学缺陷、工程审计遗留项和修复状态见 `AUDIT_STATUS.md`（已合并取代旧的 `PHYSICS_DEFECTS.md`/`RELEASE_AUDIT_REMAINING.md`/`RE_AUDIT_2026-07-10.md`，这三份旧文件现仅作为指向 `AUDIT_STATUS.md` 的兼容性存根保留）。

截至 2026-07-16 的实现快照：默认生产主链使用
...
按状态共享、带协议指纹和 OpenMM checkpoint 的 fixed-H 探针轨迹库；MBAR 校准后的
冻结 `f_k` 若尚未通过独立验证，会以 `calibrated_pending_validation` 状态保存，并按
50k→150k→300k 的累计预算阶梯续验，不再回退 SGD、盲拆窗口或重复执行昂贵探针。
当前没有剩余的、已确认会阻断默认生产主链的 P0/P1；未修代码行动以 `todolist.md`
为准，代码已修但尚待真实 GPU/完整依赖验证的项目以 `VALIDATION_MATRIX.md` 为准。
...
AUDIT_STATUS.md         当前权威审计状态：影响 Delta G 数值的物理/建模问题 + 工程审计遗留清单
todolist.md             尚未实现、仍需修改代码或采取工程行动的清单
VALIDATION_MATRIX.md    代码已修但尚待完整测试/GPU 实测的验证监控表
PHYSICS_DEFECTS.md      已合并入 AUDIT_STATUS.md，现仅为兼容性存根
RELEASE_AUDIT_REMAINING.md  已合并入 AUDIT_STATUS.md，现仅为兼容性存根
```
...
注意：仓库里的 `gmx_path` 是机器相关路径，换机器运行前必须检查。当前配置刻意保留
`enable_lambda_refine=false`，以免覆盖已有的 Stage 2 尾部局部修复；不要在不了解
`abfe_config.json` 中 `_comment_lambda_refine` 背景的情况下直接打开。
...
- `lj_long_range_dispersion_correction.status` 是否为 `implemented_analytic_mean_field_switching_softcore_aware`（`protocol_version=2`），并核对相应 LRC 协议版本/指纹；旧的 `not_implemented` 或 `implemented_analytic_mean_field`（v1，未包含 switching 区间修正）输出不得与新协议结果混用。
...
这是历史 provenance 文本缓存造成的已知问题。当前 README 和 `AUDIT_STATUS.md` 的结论优先：APBS 不替代 LJ tail correction，手动 PME self `+C*lambda^2` 不作为生产修正项，`Delta G_bind = Delta G_solvent - Delta G_complex + Delta G_APBS`（不是 `Delta G_complex - Delta G_solvent`）。如果手头的 `output/final_binding_results.json`/`thermodynamic_cycle.md` 是在这几处文档修正之前生成的，其中的 `delta_G_bind_kJ_mol` 符号可能是反的，`thermodynamic_cycle` 字段文本也会是旧版本——重新跑一遍复合物腿+溶剂腿的最终汇总（不需要重新采样，只要 `complex_results`/`solv_results` 能从缓存加载）即可刷新成当前约定。
