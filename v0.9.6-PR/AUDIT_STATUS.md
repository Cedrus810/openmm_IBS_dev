# ABFE 审计状态总表（更新至 2026-07-16）

> **当前状态摘要（2026-07-16）**：以下正文保留完整审计演进记录，因此较早章节中
> 可能仍出现当时成立、后来已经修复的“未实现”或旧协议版本描述。当前源码采用
> `IBS_BIAS_PROTOCOL_VERSION=12`、`THERMODYNAMIC_PATH_PROTOCOL_VERSION=7`、
> `TRADITIONAL_LJ_LRC_PROTOCOL_VERSION=2`、`WCA_ACCOUNTING_VERSION=2`；默认生产
> 主链目前没有剩余的已确认 P0/P1。现行未修代码行动以 `todolist.md` 为准，代码已修
> 但尚待运行验证的项目以 `VALIDATION_MATRIX.md` 为准；README 中英文版提供面向运行者
> 的精简口径。
...
- `todolist.md`：尚未实现、仍需修改代码或采取工程行动的事项。
- `VALIDATION_MATRIX.md`：代码已修但仍等待完整测试、GPU 实测或证据归档的项目。
...
## 2026-07-16 当前代码复审（IBS v10-v12 / P2 批量修复 / native resume）
...
- **默认生产路径没有剩余的已确认 P0/P1；上述 13 项 P2 和主窗口 native resume 均已代码级修复。**
...
- 本地环境缺少 OpenMM/PyMBAR，本轮只完成静态检查；相关回归测试已经补入源码，但尚未在目标运行环境完整执行。具体运行状态、验收条件和证据要求统一跟踪在 `VALIDATION_MATRIX.md`。
