...
class SourceContractTests(unittest.TestCase):
    ...
    def test_bias_protocol_and_update_count_gate_are_locked(self):
        self.assertIn("IBS_BIAS_PROTOCOL_VERSION = 12", self.engine)
    ...
    def test_fixed_h_probe_cache_protocol_version_is_explicit(self):
        self.assertIn("FIXED_H_PROBE_CACHE_PROTOCOL_VERSION = 2", self.engine)

    def test_production_section_does_not_update_weights(self):
        ...
        # Mentions in explanatory comments are allowed; an executable call is not.
        executable_lines = [
    ...
    def test_preopt_cache_requires_full_protocol_match_not_just_state_count(self):
        ...
        self.assertEqual(
            self.pipeline.count(
                "protocol_match = cached_protocol is not None and cached_protocol == _stage1_protocol_key"
            ),
            1,
        )
        self.assertEqual(
            self.pipeline.count(
                "protocol_match = cached_protocol is not None and cached_protocol == _stage2_protocol_key"
            ),
            1,
        )
        self.assertIn(
    ...
    def test_final_convergence_gate_checks_absolute_ess_and_uncertainty(self):
        ...
        self.assertIn("min_absolute_ess >= float(final_min_absolute_ess)", conv_block)
        self.assertIn("min_decorrelated_samples >= int(final_min_decorrelated_samples)", conv_block)
        self.assertIn(
            "max_endpoint_uncertainty_kJ_mol <= float(final_max_uncertainty_kJ_mol)", conv_block
        )
