...
def test_traditional_run_full_passes_resume_to_both_legs(tmp_path, resume):
    ...
    def fake_run_leg(
        ...
        *,
        ...
        return {"delta_G": 1.0, "error": 0.1, "converged": True}
...
def test_traditional_cli_disables_resume_under_reset():
    ...
    # [P1-13] 2026-08-30：两条腿各新增一处基线预平衡（complex/solvent
    # baseline），resume 接线从 2 处变为 4 处——每一处（预平衡 ×2 + REMD
    # run_full ×2）都必须同样 `and not config.reset`，计数守护防止有人
    # 新增接线时漏掉这个合取。
    assert traditional_body.count("resume=config.resume and not config.reset,") == 4
...
def _current_accounting_conv(window_idx, **overrides):
    ...
    **加上** sampling_repair_policy 四项全部匹配，任何一项不符（或读不到文件）
    ...
    conv = {
        ...
        # 第二条窗口复用路径（λ 重划分/窗口重编号）同样必须校验它。
        "ligand_com_restraint_protocol_version": ap.LIGAND_COM_RESTRAINT_PROTOCOL_VERSION,
...
def _matching_conv(**overrides):
    ...
    conv = {
        ...
        # [ligand_com_restraint_protocol_version] v1 的 Group 5 COM 限制力在 CUDA 上
        # 产生定向拖拽，那种窗口不满足平衡采样前提；缺字段一律判不可复用。
        ...
        "production_segments": [{"start_frame": 0, "end_frame": GOOD_SHAPE[1], "n_frames": GOOD_SHAPE[1], "session_id": "test", "reason": "fresh"}],
    }
    conv.update(overrides)
    return conv
...
# ---------------------------------------------------------------------------
# [ligand_com_restraint_protocol_version] Group 5 配体 COM 限制力的逐窗口缓存门
#
# stage 级协议指纹（abfe_pipeline._stage_protocol_key）会因 v1->v2 让 stage 结果
# 失效，但那之后流程会**重新进入 run_all_windows**，而逐窗口的 energies/bias/base
# 复用发生在建 Context 之前、走的是本门。少了这道门，旧 Group5 窗口的能量数组仍会
# 被直接复用。见 4W53/STAGE2_GROUP5_CUDA_PBC_ROOT_CAUSE_2026-08-29.md。
# ---------------------------------------------------------------------------


def test_resume_rejects_v1_group5_window_cache():
    """v1 缓存（该力仍在，字段值为 1）必须被拒绝。"""
    status = _gate(_matching_conv(ligand_com_restraint_protocol_version=1))
    assert status["usable"] is False
    assert status["com_restraint_version_match"] is False
    assert "ligand_com_restraint_protocol_version" in status["reason"]


def test_resume_rejects_window_cache_missing_the_field():
    """比 v1 更旧、根本没写这个字段的缓存，同样必须 fail-closed。"""
    conv = _matching_conv()
    conv.pop("ligand_com_restraint_protocol_version")
    status = _gate(conv)
    assert status["usable"] is False
    assert status["com_restraint_version_match"] is False


@pytest.mark.parametrize("stage_type", ["coul", "vdw"])
def test_group5_gate_applies_to_both_stages(stage_type):
    """该力只在没有 Boresch 锚定时添加，即溶剂腿 stage1/stage2 **都**有，
    所以这道门不做 stage_type 豁免（与 vdw_nonbonded_protocol_version 不同）。"""
    conv = _matching_conv(
        ligand_com_restraint_protocol_version=1,
        vdw_nonbonded_protocol_version=ie.VDW_NONBONDED_PROTOCOL_VERSION,
    )
    status = _gate(conv, stage_type=stage_type)
    assert status["usable"] is False, f"{stage_type} 也必须拒绝 v1 缓存"
    assert status["com_restraint_version_match"] is False


def test_current_version_window_cache_is_accepted():
    """v2 缓存（力已移除）必须可复用，否则每次 resume 都白重采。"""
    conv = _matching_conv(
        ligand_com_restraint_protocol_version=ie.LIGAND_COM_RESTRAINT_PROTOCOL_VERSION
    )
    status = _gate(conv)
    assert status["usable"] is True, status
    assert status["com_restraint_version_match"] is True


def test_window_manifest_writer_records_the_version():
    """采样落盘的 convergence.json 必须写入该字段，否则新窗口自己也过不了门。"""
    from pathlib import Path

    src = (Path(ie.__file__).resolve()).read_text(encoding="utf-8")
    assert '"ligand_com_restraint_protocol_version": (' in src
    assert "LIGAND_COM_RESTRAINT_PROTOCOL_VERSION\n                ),\n" in src or \
           "LIGAND_COM_RESTRAINT_PROTOCOL_VERSION" in src


def test_second_reuse_path_also_rejects_v1_group5_windows(tmp_path, monkeypatch):
    """`_old_window_accounting_ok`（λ 重划分时的窗口复用）也必须拒绝 v1 缓存。

    两条路径都拦住，才算 Group5 的逐窗口缓存 fail-closed 真的闭合。
    """
    import inspect

    src = inspect.getsource(ap.ABFEPipeline._run_dual_lambda_stage) \
        if hasattr(ap.ABFEPipeline, "_run_dual_lambda_stage") else ""
    full = (Path(ap.__file__).resolve()).read_text(encoding="utf-8")
    i = full.index("def _old_window_accounting_ok(")
    j = full.index("reuse_map: Dict[int, int] = {}", i)
    body = full[i:j]
