...
def _write_window(output_dir, checkpoint_dir, local_idx, stage_type="vdw", seed=0):
    ...
    from production_artifact_fixtures import identify_window
    start, end = RANGES[local_idx]
    identify_window(output_dir, checkpoint_dir, local_idx, LAMBDAS_COUL[start:end], LAMBDAS_VDW[start:end], kind=stage_type)
    return paths
...
def test_rescue_merge_positive_case(tmp_path):
    ...
    from production_artifact_fixtures import identify_window
    identify_window(rescue_dir, rescue_ckpt, 0, LAMBDAS_COUL[2:5], LAMBDAS_VDW[2:5])
