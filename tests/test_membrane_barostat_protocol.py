"""B1：`system_type=membrane` + 膜恒压器（memtodolist.md §3.1 / §3.2 / §7.4 / §7.7）。
...
def test_interrupted_pre_equilibration_is_not_mistaken_for_a_finished_one(tmp_path):
    ...
    def _make(status, achieved, target=50_000_000, with_state=True):
        ...
        (d / "pre_equilibration.dcd").write_bytes(b"x" * 20000)
        (d / "checkpoints" / "pre_equil.chk").write_bytes(b"y" * 1024)
    ...
    assert runabfe.equilibrium_is_done(
        _make("completed", 50_000_000), expected_fingerprint=fp
    ) is True

    # 被中断（状态还是 running）→ 不许复用
    assert runabfe.equilibrium_is_done(
        _make("running", 20_000_000), expected_fingerprint=fp
    ) is False

    # 声称 completed 但步数不够目标（例如目标从 10 ns 调到 100 ns）→ 不许复用
    assert runabfe.equilibrium_is_done(
        _make("completed", 5_000_000), expected_fingerprint=fp
    ) is False

    # 连 pipeline_state.json 都没有 → 无法证明跑完，保守视为未完成
    assert runabfe.equilibrium_is_done(
        _make("completed", 50_000_000, with_state=False), expected_fingerprint=fp
    ) is False
...
def test_pre_equilibration_checkpoint_interval_bounds_the_work_lost_on_resume():
    ...
    # 续跑必须**追加** DCD 而不是重开一条，否则已跑的部分直接作废。
    assert "append=resume_from_chk" in body
