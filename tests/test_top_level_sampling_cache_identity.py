...
def _fake_pipeline(tmp_path, scheme, coion, *, completed=False):
    ...
    pipe.charge_treatment = "neutral"
    pipe.resolve_co_alchemical_ion_spec = lambda: None
    pipe.results = {}
    ...
    def _run_stage(**kwargs):
        ...
        # [P1-11] 真实的 `_run_2d_lambda_stage` 会透传 MBAR 的收敛状态，且
        # converged=False 的结果根本不会被写缓存/标 completed。mock 也必须
        # 代表"已收敛"的运行，否则会被同一份 sanity gate 正确地拒绝。
        return {
            "total_delta_G": 1.0,
            "total_error": 0.1,
            "converged": True,
