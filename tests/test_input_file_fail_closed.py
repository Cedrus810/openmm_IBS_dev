...
def test_underscore_prefixed_comment_keys_are_not_flagged(tmp_path, monkeypatch):
    """`abfe_config.json` 里大量用 `_comment*` 字段做人读注释——这些不是真正的
    配置字段，合并时必须继续被跳过，不能污染 preset。"""
    path = tmp_path / "config.json"
    path.write_text(
        '{"_comment": "仅供人读", "temperature": 310.0}', encoding="utf-8"
    )
    monkeypatch.setattr("sys.argv", ["runabfe.py", "--config", str(path)])
    config = RunConfig(_minimal_args(str(path)))
