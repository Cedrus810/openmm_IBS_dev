...
def test_production_modules_do_not_import_standalone_neural_module():
    ...
    for filename in (
        ...
        # 🔑 [2026-08-31] 判"有没有 import"，不是"源码文本里有没有出现这个名字"。
        # 原来直接 `assert "outer_lambda_neural_basis" not in source`：ibs_engine.py
        # 里有一条**注释**引用了 `outer_lambda_neural_basis.
        # OuterLambdaResidualBiasForce.validate_wiring`（记录 EXP-025 G4 Layer-1
        # 踩过的 cv_k_int/cv_k_rest 未注册那个坑），于是这条契约在真正的不变量
        # （生产模块不得依赖该独立模块）完全成立的情况下报红。契约本身要守的是
        # 依赖关系，注释不构成依赖。
        source = (repo_root / filename).read_text(encoding="utf-8")
        tree = ast.parse(source, filename=str(repo_root / filename))
        imported = set()
        ...
        assert "outer_lambda_neural_basis" not in imported, (
            f"{filename} 真的 import 了 outer_lambda_neural_basis"
        )
