"""B2：`charge_treatment` 与 §1.2 的 fail-closed 矩阵（docs/status/memtodolist.md §1.2 / §2.2 / §7.1）。
...
## B3/B4 已落地；生产资格仍由 C4/C5 独立控制（2026-08-13 更新）

charging Hamiltonian 与 charge-transfer 溶剂腿 builder 都已经实现，
closes_thermodynamic_cycle 必须为 true。实现完整不等于生产验收完成：
C1-C3/MEM-00h 只证明端点、接缝和数值恒等式；在 C4 smoke test 与 C5 全链验收
完成前，输出必须继续明确带 feature_status=experimental 与
production_qualified=false，且所有正式结果出口（包含 analyze-only）必须保留
同一 qualification payload。
...
def test_charged_ligand_with_coion_requires_reservoir_correction_for_closure():
    ...
    但 tethered carrier 的 reservoir-release 项仍需 C4/C5 证据；解析结果必须
    明确标记为未闭合，不能只因为 builder 存在就放行生产。
    """
    spec = _coion(+1.0)
    # 规格本身合法：单独校验不抛错。
    ...
    # B4 已落地，但 carrier reservoir correction 尚未提供，循环必须保持未闭合。
