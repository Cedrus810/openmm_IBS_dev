...
def test_legacy_broad_preopt_cache_migrates_only_under_original_hamiltonian(monkeypatch):
    monkeypatch.setattr(pipeline, "PREOPT_HAMILTONIAN_PROTOCOL_VERSION", 1)
    pipe = _fake_pipeline({"n_states_per_stage": 12})
    fresh = _key(pipe)
