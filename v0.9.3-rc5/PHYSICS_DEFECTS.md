...
## 2. 🔴 撤销：所谓"PME 自能修正"（+C·λ²）本身就是错的，已改为仅诊断、不再应用 —— 之前两版都在同一个概念错误上反复横跳
**位置**：`ibs_engine.py::TraditionalMBARAnalyzer.compute_u_kn`（`is_pme_coulomb_leg`/`is_mixed_pme_alchemical` 分支，约 4151-4229 行）；`pme_offset_charge_square_sum()`（`ibs_engine.py:188-208` 附近）；`pme_self_correction_prefactor_kj`/`pme_self_correction_energy_kj`（`ibs_engine.py:158-186` 附近）。

**这一条之前两版的记录都是错的，现在实测数据反证了它**：上一版的说法是"对于中性配体，代码会正确地对总 PME 能量加回 `+C·λ²` 来消除 OpenMM 在做电荷线性 offset 时引入的坐标无关自能伪项"，只是"带电配体分支没跟上"。**这个前提本身就不成立**：`NonbondedForce.addParticleParameterOffset` 是 OpenMM 官方支持的线性电荷缩放机制，每次 `getState(getEnergy=True)` 都会用当前 λ 对应的真实（已缩放）电荷重新计算包含 Ewald self-energy（`-ke·alpha/√π·Σq_i²`）在内的完整 PME 能量——这个 self-energy 项是该 λ 态总哈密顿量里真实存在、且必须存在的一部分（它精确抵消了倒易空间求和对粒子自身的重复计数，不是数值伪影）。MBAR/TMBAR 只要求 `U_k(x)` 是状态 k 的正确势能，self-energy 项本就应该完整保留在 `U_k` 里参与自由能估计。手动加回 `+C·λ²` 相当于凭空反向抵消了一个真实存在的物理量。

**实测证据**：本条修正此前从未真正跑通过（要么因为带净电荷分支被禁用直接跳过，要么因为 `pme_offset_charge_square_sum` 里的 `.value_in_unit()` 假设与实际 OpenMM 8.5.1 API 返回类型不符而直接抛 `AttributeError` 崩溃）。修完那个崩溃、这条"修正"第一次真正跑到底之后，中性 Atenolol 的 decharging 腿给出 `ΔG_charge = -954.81 kJ/mol`（约 -228 kcal/mol）——对一个把配体电荷从环境中"关掉"的过程而言明显偏大了 2-3 倍。按 `Σq_offset²≈5.03 e²` 反推，这条 `+C·λ²` 修正本身的量级正好是几百 kJ/mol，与"偏大的部分"数量级吻合。

**修复状态**：撤销修正，只保留诊断。`pme_offset_charge_square_sum()` 仍会计算并记录 `Σq_offset²`（对排查/存档有用，写入 `diagnostics.pme_self_correction.charge_square_sum_e2`），但 `apply_pme_self_correction` 永远不再被置为 `True`，不再把这个量转换成 `+C·λ²` 能量项叠加进 u_kn。诊断字段里新增 `applied: false` + `note` 说明原因。

**是否影响本次运行**：**是**——这是当前中性 Atenolol 体系 decharging 腿结果明显偏大的主因（并非本条最初版本所说的"仅带电配体分支未触发的潜在缺陷"）。

**遗留问题**：撤销这条"修正"之后，decharging 腿的 ΔG 量级回落到大致合理区间，但具体数值仍需要一次干净的重跑来最终确认；同时 vanishing（去 VDW）腿在同一次运行中 `min_overlap_proxy≈0.009`（拼接曲线单步跳变约 100+ kJ/mol）仍然偏低，需要检查是否需要增加该腿的窗口密度/采样长度，这不是本条修正范围内的问题。
...
| 2 | "PME 自能修正"（+C·λ²） | 🔴 撤销，改为仅诊断 | 该修正本身概念有误，会反向抵消真实存在的 Ewald self-energy；实测导致中性配体 decharging 腿 ΔG 明显偏大，已停用 |
