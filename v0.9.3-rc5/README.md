...
- VDW/vanishing 腿的 LJ 长程色散 tail/LRC 修正尚未实现；结果 JSON 中明确记录为 `lj_long_range_dispersion_correction.status = not_implemented`。
- APBS 修正只作为最终外部项 `Delta G_APBS` 加到 `Delta G_bind`，不能替代 LJ tail correction。
- 旧输出中的 `thermodynamic_cycle.md` 和 provenance 里仍可能包含历史 PME self-correction 描述；请以 `PHYSICS_DEFECTS.md` 和当前 diagnostics 为准。当前结论是：手动 `+C*lambda^2` PME 自能“修正”已撤销，不应作为生产修正项使用。
- 当前 Boresch 谐振性校验通过，但力常数有裁剪警告，需要在结果解释中保留。
- Stage 2 采用 `Local-TMBAR-Stitched`，误差已传播窗口 offset 方差，但尚未包含完整全局 MBAR 协方差、自相关时间和有效样本数修正。
