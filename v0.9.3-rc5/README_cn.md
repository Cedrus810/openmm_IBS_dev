...
得到跟当前公式匹配的号（详见下方“结果解读”和 `PHYSICS_DEFECTS.md`）。** 下面这组
...
- VDW/vanishing 腿的 LJ 长程色散 tail/LRC 修正尚未实现；`output/final_results.json` 和 `output/solvent_leg/final_results.json` 里 `lj_long_range_dispersion_correction.status` 均为 `not_implemented`。
- APBS 修正只作为最终外部项 `Delta G_APBS` 加到 `Delta G_bind`（当前 `apbs_correction_kJ_mol = 0.0`，未启用），不能替代 LJ tail correction。
- 旧输出中的 `thermodynamic_cycle.md` 和 provenance 里仍可能包含历史 PME self-correction 描述；`output/final_binding_results.json` 里 `provenance.thermodynamic_cycle` 目前就是这种未刷新的旧文本快照。请以 `PHYSICS_DEFECTS.md` 和当前 diagnostics 为准。当前结论是：手动 `+C*lambda^2` PME 自能“修正”已撤销，不应作为生产修正项使用。
...
更详细的方法学缺陷和修复状态见 `PHYSICS_DEFECTS.md`。工程审计遗留项见 `RELEASE_AUDIT_REMAINING.md`。
...
PHYSICS_DEFECTS.md      影响 Delta G 数值的物理/建模问题清单
RELEASE_AUDIT_REMAINING.md  工程审计遗留清单
```
...
注意：仓库里的 `gmx_path` 是机器相关路径，换机器运行前必须检查。
...
- `lj_long_range_dispersion_correction.status` 是否仍为 `not_implemented`。
...
如果 self-test 与 `PHYSICS_DEFECTS.md` 的最新物理结论不一致，应更新测试和热力学循环文档，避免旧假设继续进入新 provenance。

推荐下一步优先事项：

1. 实现或明确外部处理 LJ tail/LRC 修正。
2. 更新 `abfe_core.py::THERMODYNAMIC_CYCLE_DOC` 中历史 PME self-correction 文本。
3. 修正 self-test 中历史 PME self-correction sign 检查。
4. 对当前 Atenolol-rank11 配置做至少一次独立重复运行。
5. 根据 stage diagnostics 判断是否需要进一步加密 vanishing 阶段窗口或增加采样。
